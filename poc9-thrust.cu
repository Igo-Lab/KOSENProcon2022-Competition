...
__global__ void reduce_problem(short *problem, short *src, unsigned int *sum, const int clip_starti, const int clip_endi, const int problemLen) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    // zip iterator
    *sum[idx] += thrust::reduce(thrust::cuda::par, problem, problem + clip_starti, 0, AbsPlus<int, unsigned int>());
    *sum[idx] += thrust::reduce(thrust::cuda::par, problem + clip_endi, problem + problemLen, 0, AbsPlus<int, unsigned int>());
}
